"""AG News DistilBERT example using the pulseopt PyPI package.

Install once:
    pip install pulseopt torch transformers datasets

Run:
    python task_agnews.py --epochs 3 --output run.log

AG News only ships train/test splits, so this script carves a 10% validation
split out of the training set with a fixed seed.
"""
...
def build_dataloaders(
    ...
    tokenized = raw.map(tokenize, batched=True, desc="Tokenizing AG News")
...
def main() -> None:
    ...
    log_lines: list[str] = []
    header = (
        f"AG News {args.model_name} + AEES (AdamW)\n"
