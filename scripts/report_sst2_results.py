...
DEFAULT_RESULT_DIRS = [
    "results/sst2_final_warmup_linear_main"
]
...
def summarize_run(path: Path) -> RunSummary | None:
    ...
    if "sst2" not in task_name and "sst-2" not in dataset_name and "sst2" not in str(path):
        return None
...
def main() -> None:
    ...
    parser.add_argument(
        "--csv",
        type=Path,
        default=Path("results/sst2_summary.csv"),
